...
def set_regularization(model,
    ...
    for layer in model.layers:
        ...
        if kernel_regularizer and hasattr(layer, 'kernel_regularizer'):
            layer.add_loss(keras.regularizers.l2(0.0001)(layer.kernel))
...
def build_pspnet(nb_classes, resnet_layers, input_shape, freeze):
    ...
    model = set_regularization(model, kernel_regularizer=True)

    unfrozen_list=['conv5_3_pool1_conv', 'conv5_3_pool2_conv', 'conv5_3_pool3_conv', 'conv5_3_pool6_conv', 'conv6', 'conv5_4']
    if(freeze):
        for layers in model.layers:
            if layers.name not in unfrozen_list:
